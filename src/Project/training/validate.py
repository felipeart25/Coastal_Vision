...
def validate(model, val_loader, criterion, device, epoch, eval_table_data, future_seq=1):
    ...
    with torch.no_grad():
        for batch_idx, (slow_input, target_seq, fast_input, fast_fut_input) in enumerate(val_loader):
            ...
            if criterion.__class__.__name__ == "UnifiedFocalLoss" or criterion.__class__.__name__ == "DiceLoss":
        
                loss = criterion(output, target_seq)

            elif criterion.__class__.__name__ == "BufferDiceLoss" or criterion.__class__.__name__ == "HybridDiceLoss":
                buffer = buffer_mask_vec(target_seq, buffer_size=5)
                loss = criterion(output, target_seq, buffer)
