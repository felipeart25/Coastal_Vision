...
class BufferDiceLoss(nn.Module):
    def __init__(self, smooth=1e-6, apply_sigmoid=True):
        super(BufferDiceLoss, self).__init__()
        self.smooth = smooth
        self.apply_sigmoid = apply_sigmoid

    def forward(self, inputs, targets, buffer_mask):
        if self.apply_sigmoid:
            inputs = torch.sigmoid(inputs)

        if inputs.dim() == 4:
            inputs = inputs.unsqueeze(1)  # → (B, 1, 1, H, W)
        if targets.dim() == 4:
            targets = targets.unsqueeze(1)

        # Apply buffer mask
        inputs = inputs * buffer_mask
        targets = targets * buffer_mask
...
class HybridDiceLoss(nn.Module):
    def __init__(self, alpha=0.7, smooth=1e-6):
        super(HybridDiceLoss, self).__init__()
        self.buffer_dice = BufferDiceLoss(smooth=smooth)
        self.global_dice = DiceLoss(smooth=smooth, apply_sigmoid=True)
        self.alpha = alpha

    def forward(self, inputs, targets, buffer_mask):
        loss_buffer = self.buffer_dice(inputs, targets, buffer_mask)
        loss_global = self.global_dice(inputs, targets)
        return self.alpha * loss_buffer + (1 - self.alpha) * loss_global
...
def compute_area_changes(pred, target, pixel_area=400):
    """
    Computes accretion (FP) and erosion (FN) areas between prediction and target binary masks.

    Args:
        pred (Tensor): [1, H, W] binary prediction (0 or 1)
        target (Tensor): [1, H, W] ground truth (0 or 1)
        pixel_area (float): area per pixel (e.g., 1m² or 0.25m²)

    Returns:
        accretion_area (float), erosion_area (float)
    """
    ...
    fp = (pred == 1) & (target == 0)  # Accretion
    fn = (pred == 0) & (target == 1)  # Erosion
    ...
    relative_overpredicted = (overpredicted / land_area) * 100 if land_area > 0 else 0
    relative_underpredicted = (underpredicted / land_area) * 100 if land_area > 0 else 0
    ...
    return overpredicted, underpredicted, land_area, relative_overpredicted, relative_underpredicted
