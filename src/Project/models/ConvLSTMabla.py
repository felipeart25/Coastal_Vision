...
class Predictor(nn.Module):
    def __init__(self, input_dim, hidden_dims, kernel_sizes, num_layers, lstm_hidden_size, dropout, wave_steps_per_frame, future_seq=1, use_wave_context=True):
        ##### Initialize parameters ####
        ...
        self.use_wave_context = use_wave_context
    ...
    def forward(self, slow_input, fast_input, fast_fut_input, future_seq=1, inference=False):
        ...
        if self.use_wave_context:
            #### Process wave stream + future wave data ####
            print('1. Processing wave stream and future wave data...')
            wave_context = self.wave_encoder(fast_input, fast_fut_input)
        ...
        for t in range(future_seq):
            ...
            if self.use_wave_context:
                ...
                pred = self.activation(current_input.unsqueeze(1))  # (B, 1, C, H, W)
