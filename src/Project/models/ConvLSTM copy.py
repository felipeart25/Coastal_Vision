...
class Predictor(nn.Module):
    ...
    def forward(self, slow_input, fast_input, fast_fut_input, future_seq=1, inference=False):
        ...
        for t in range(future_seq):
            ...
            hidden_state = [(h.detach(), c.detach()) for h, c in hidden_state]  # Detach to prevent backprop through entire sequence
            current_input = self.conv_output(lstm_output[:, 0])

            #### Concatenate ConvLSTM output and wave context ####
            fused = torch.cat([
                lstm_output.squeeze(1),       # (B, hidden_dim, H, W)
                current_wave.squeeze(1)       # (B, wave_dim, H, W)
            ], dim=1)    
