...
# SSIM
ssim = torchmetrics.image.ssim.StructuralSimilarityIndexMeasure(data_range=1.0)
...
def buffer_mask(y_true_batch, buffer_size=10):
    """
    Generates shoreline buffer masks for each sample in a batch.

    Args:
        y_true_batch (Tensor): (B, 1, H, W) ground truth binary masks
        buffer_size (int): pixel width of the buffer zone

    Returns:
        buffer_masks (Tensor): (B, H, W) binary masks indicating buffer zones
    """
    y_true_np = y_true_batch.squeeze(1).cpu().numpy()  # Shape: (B, H, W)
    buffer_masks = []

    for i in range(y_true_np.shape[0]):
        gt = y_true_np[i].astype(np.float32)

        # Compute shoreline edges
        grad_y, grad_x = np.gradient(gt)
        edge_map = (np.abs(grad_y) + np.abs(grad_x)) > 0

        # Dilate to create buffer zone
        buffer = binary_dilation(edge_map, iterations=buffer_size)
        buffer_masks.append(buffer)

    return torch.tensor(np.stack(buffer_masks), dtype=torch.bool, device=y_true_batch.device)  # (B, H, W)
...
def dice_score_shoreline_buffer(y_pred, y_true, buffer_mask, delta=0.5, eps=1e-6):
    ...
    buffer_masks = buffer_mask

    B = y_true.size(0)
    scores = []

    for i in range(B):
        pred_i = y_pred_bin[i, 0].squeeze()
        true_i = y_true[i, 0]
        mask = buffer_masks[i]
...
def buffer_zone_metrics(y_pred, y_true, buffer_masks, eps=1e-6):
    """
