...
def validate(model, val_loader, criterion2, device, epoch, eval_table_data, future_seq=1):
    ...
    with torch.no_grad():
        for batch_idx, (slow_input, target_seq, fast_input, fast_fut_input) in enumerate(val_loader):
            ...
            loss = criterion2(output, target_seq)
            val_loss += loss.item()
...
def main():
    ...
    wandb.init(project="THESIS",
        name=f"hybrid_v14_30Epochs_case1_biweekly_10fut",
        config={
        "input_dim": 1,
        "hidden_dims": [ 64, 64],
    ...
    plot_path = os.path.join(folder_outputs, "loss_curves.png")
    plt.savefig(plot_path, dpi=300)
    plt.close()
    
    # Log loss curves to WandB
    wandb.log({"Loss Curves": wandb.Image(plot_path)})
