...
from metrics import mae_loss, pearson_corr, ssim, DiceLoss, compute_iou, compute_precision, compute_recall, UnifiedFocalLoss
...
wandb.init(project="THESIS",
    name=f"hybrid_v9_daily-{wandb.util.generate_id()}",
    config={
    "input_dim": 1,
    "hidden_dims": [ 64, 64],
...
train_loader, val_loader, test_loader = data_loaders(data_path, batch_size=wandb.config.batch_size, time="bi-monthly", case=1)
...
def validate(model, val_loader, criterion, device, epoch, eval_table_data):
    model.eval()
    ssim.to(device)
    val_loss = 0
    mae_total = 0
    ...
    num_timesteps = 10
    mae_totals = [0.0] * num_timesteps
    ssim_totals = [0.0] * num_timesteps
    dice_totals = [0.0] * num_timesteps
    pearson_totals = [0.0] * num_timesteps
    iou_totals = [0.0] * num_timesteps
    precision_totals = [0.0] * num_timesteps
    recall_totals = [0.0] * num_timesteps
    total_pixels = [0] * num_timesteps
    ...
    with torch.no_grad():
        for (slow_input, target_seq, fast_input, fast_fut_input) in val_loader:  
            ...
            mae = mae_loss(output, target_seq)
            pearson = pearson_corr(output, target_seq)
            
            ssim_scores = []
            for t in range(output.shape[1]):  # over time
                ssim_score_t = ssim(output[:, t], target_seq[:, t])
                ssim_scores.append(ssim_score_t)
            ssim_score = torch.stack(ssim_scores).mean()

            mae_total += mae.item()
            pearson_total += pearson.item()
            ssim_total += ssim_score.item()

            for t in range(num_timesteps):
                out_t = output[:, t, 0].unsqueeze(1)  # (B, 1, H, W)
                tgt_t = target_seq[:, t, 0].unsqueeze(1)

                # === MAE === (sum absolute error per pixel)
                mae = torch.abs(out_t - tgt_t).sum().item()
                mae_totals[t] += mae
                total_pixels[t] += B * H * W

                # === Other metrics === (mean over samples)
                dice = 1 - GeneralizedDiceLoss(sigmoid=False)(out_t, tgt_t).item()
                dice_totals[t] += dice
                ssim_val = ssim(out_t, tgt_t).item()
                ssim_totals[t] += ssim_val
                pearson = pearson_corr(out_t, tgt_t).item()
                pearson_totals[t] += pearson
                iou = compute_iou(out_t, tgt_t).item()
                iou_totals[t] += iou
                prec = compute_precision(out_t, tgt_t).item()
                precision_totals[t] += prec
                rec = compute_recall(out_t, tgt_t).item()
                recall_totals[t] += rec
    ...
    avg_pearson = pearson_total / len(val_loader)
    avg_ssim = ssim_total / len(val_loader)
    


    # === Average per timestep ===
    avg_mae_per_t = [mae_totals[t] / total_pixels[t] for t in range(num_timesteps)]
    avg_dice_per_t = [dice_totals[t] / sample_count for t in range(num_timesteps)]
    avg_ssim_per_t = [ssim_totals[t] / sample_count for t in range(num_timesteps)]
    avg_pearson_per_t = [pearson_totals[t] / sample_count for t in range(num_timesteps)]
    avg_iou_per_t = [iou_totals[t] / sample_count for t in range(num_timesteps)]
    avg_precision_per_t = [precision_totals[t] / sample_count for t in range(num_timesteps)]
    avg_recall_per_t = [recall_totals[t] / sample_count for t in range(num_timesteps)]
    ...
    metric_rows = {
        ...
        "SSIM": avg_ssim_per_t,
        "Dice": avg_dice_per_t,
        "Pearson": avg_pearson_per_t,
        "IoU": avg_iou_per_t,
        "Precision": avg_precision_per_t,
        "Recall": avg_recall_per_t
        }

    # Append each metric as a row in the table
    for metric_name, values in metric_rows.items():
        row = [metric_name] + values + [epoch]
        eval_table_data.append(row)

    # Log metrics per frame to wandb
    # for t in range(num_timesteps):
    #     wandb.log({
    #         f"MAE_t{t+1}": avg_mae_per_t[t],
    #         f"SSIM_t{t+1}": avg_ssim_per_t[t],
    #         f"Dice_t{t+1}": avg_dice_per_t[t],
    #         f"Pearson_t{t+1}": avg_pearson_per_t[t],
    #         f"IoU_t{t+1}": avg_iou_per_t[t],
    #         f"Precision_t{t+1}": avg_precision_per_t[t],
    #         f"Recall_t{t+1}": avg_recall_per_t[t],
    #         "Epoch": epoch
    #     })

    wandb.log({
    "Avg MAE (All Timesteps)": np.mean(avg_mae_per_t),
    "Avg SSIM (All Timesteps)": np.mean(avg_ssim_per_t),
    "Avg Dice (All Timesteps)": np.mean(avg_dice_per_t),
    "Avg Pearson (All Timesteps)": np.mean(avg_pearson_per_t),
    "Epoch IoU": np.mean(avg_iou_per_t),
    "Epoch Precision": np.mean(avg_precision_per_t),
    "Epoch Recall": np.mean(avg_recall_per_t),
    "Epoch": epoch
    })
...
def visualize_prediction(model, test_loader, device, sample_idx=0):
    ...
    # Target sequence
    for t in range(10):
        axes[0, t].imshow(target_seq[0, t, 0].cpu().numpy(), cmap='gray', origin='lower')
        axes[0, t].set_title(f'Target t={t+10}')
        axes[0, t].axis('off')
    
    # Predicted sequence
    for t in range(10):
        axes[1, t].imshow(output[0, t, 0].cpu().numpy(), cmap='gray', origin='lower')
        axes[1, t].set_title(f'Pred t={t+10}')
        axes[1, t].axis('off')

    for t in range(10):
        pred = (output[0, t, 0] > 0.5).float()
        target = target_seq[0, t, 0]

        fp = (pred == 1) & (target == 0)  # False Positives
        fn = (pred == 0) & (target == 1)  # False Negatives

        print("FP shape:", fp.shape)  # Debugging line
        print("FN shape:", fn.shape)  # Debugging line

        axes[2,t].imshow(target.cpu().numpy(), cmap='gray', origin='lower')  # base image
        axes[2,t].imshow(fn.cpu().numpy(), cmap='Reds', alpha=0.6, origin='lower')   # red = false negatives
        axes[2,t].imshow(fp.cpu().numpy(), cmap='Greens', alpha=0.6, origin='lower') # green = false positives
        axes[2,t].set_title(f'Error t={t+10}')
        axes[2,t].axis('off')
...
def test_metrics(model, test_loader, device, test_data):
    model.eval()
    ssim.to(device)
    ...
    precision_totals = [0.0] * num_timesteps
    recall_totals = [0.0] * num_timesteps
    fp_totals = [0.0] * num_timesteps
    fn_totals = [0.0] * num_timesteps 
    total_pixels = [0] * num_timesteps

    sample_count = 0

    with torch.no_grad():
        for (slow_input, target_seq, fast_input, fast_fut_input) in test_loader:
            slow_input = slow_input.to(device)
            fast_input = fast_input.to(device)
            target_seq = target_seq.to(device)
            fast_fut_input = fast_fut_input.to(device)
            ...
            for t in range(num_timesteps):
                    ...
                    pred_bin = (out_t > 0.5).float()
                    tgt_bin = (tgt_t > 0.5).float()
    ...
    avg_precision_per_t = [precision_totals[t] / sample_count for t in range(num_timesteps)]
    avg_recall_per_t = [recall_totals[t] / sample_count for t in range(num_timesteps)]
    avg_fp_per_t = [fp_totals[t] / sample_count for t in range(num_timesteps)]
    avg_fn_per_t = [fn_totals[t] / sample_count for t in range(num_timesteps)]
...
def main():
    ...
    # Loss function
    criterion = UnifiedFocalLoss(weight=0.5, delta=0.7, gamma=0.75)
    ...
    for epoch in range(1, epochs + 1):
        ...
        val_loss = validate(model, test_loader, criterion, device, epoch, eval_table_data)
    ...
    table_columns = ["Metric"] + [f"t={i+1}" for i in range(10)] + ["Epoch"]
    table_columns2 = ["Metric"] + [f"t={i+1}" for i in range(10)]
