...
wandb.init(project="convlstm-mnist", 
           name=f"SAConvLSTMv34-{wandb.util.generate_id()}",
           config={
    "input_dim": 1,
...
class SimpleSAConvLSTM(nn.Module):
    ...
    def __init__(self, input_dim=1, hidden_dims=[64], num_layers=4, kernel_sizes=[(3,3)]):
        super().__init__()
        self.input_dim = input_dim
        ...
        for i in range(num_layers):
            cell_input_dim = input_dim if i == 0 else hidden_dims[i - 1]
            self.cells.append(SAConvLSTMCell(cell_input_dim, hidden_dims[i], kernel_sizes[i]))

        
        # Output layer: map hidden state back to input dimension
        self.conv_out = nn.Conv2d(in_channels=self.hidden_dims[-1], out_channels=input_dim, kernel_size=1)
        
    def init_hidden(self, batch_size, height, width, device):
        """Initialize hidden, cell and memory states for all layers"""
        hidden = []
        for i in range(self.num_layers):
            h = torch.zeros(batch_size, self.hidden_dims[i], height, width, device=device)
            c = torch.zeros(batch_size, self.hidden_dims[i], height, width, device=device)
            m = torch.zeros(batch_size, self.hidden_dims[i], height, width, device=device)
            hidden.append((h, c, m))
        return hidden
