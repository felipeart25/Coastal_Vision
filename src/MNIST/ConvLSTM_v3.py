...
def train(model, train_loader, criterion, optimizer, device, epoch):
    ...
    for batch_idx, (input_seq, future_seq) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast():
            ...
            outputs = []
            current_input = input_seq.clone()  # Clone to avoid modifying original tensor
            
            for t in range(seq_len):
                # 🔹 Decide between teacher forcing or prediction **before** running the model
                use_teacher_forcing = torch.rand(1).item() < teacher_forcing_prob
                if t == 0 or use_teacher_forcing:
                    next_input = future_seq[:, t]  # Ground truth
                else:
                    with torch.no_grad():  # No gradient needed for previous outputs
                        next_input = model(current_input)[:, -1]  # Predict next frame
                
                # Append predicted frame to outputs
                outputs.append(next_input.unsqueeze(1))
                
                # Update input sequence with the chosen frame
                current_input = torch.cat([current_input[:, 1:], next_input.unsqueeze(1)], dim=1)

            # Stack outputs along time dimension
            outputs = torch.cat(outputs, dim=1)

            loss = criterion(outputs, future_seq)
        ...
        iteration_time = start_iter.elapsed_time(end_iter) / 1000  # Convert ms to s
